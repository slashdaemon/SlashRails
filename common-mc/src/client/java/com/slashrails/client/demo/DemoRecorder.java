...
final class DemoRecorder {
    ...
    private DemoRecorder(int width, int height, int fps, Process ffmpeg) {
        ...
        writer = new Thread(() -> {
            try {
                ...
                SlashRails.LOG.error("[demo] recorder writer failed", e);
            }
        }, "SlashRails demo recorder");
    ...
    static DemoRecorder start(Path ffmpegExe, Path output, int fps) throws IOException {
        ...
        SlashRails.LOG.info("[demo] recording {}x{} @{} fps -> {}", w, h, fps, output);
    ...
    void stop() {
        ...
        SlashRails.LOG.info("[demo] recording finished: {} frames ({} repeated to keep real time), ffmpeg exit {}",
