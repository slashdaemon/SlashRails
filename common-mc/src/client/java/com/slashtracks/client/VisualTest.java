package com.slashtracks.client;
...
/**
 * Dev-only scripted screenshot run, enabled with {@code -Dslashtracks.visualtest=true} and a world
 * opened by quick play. Builds fixtures, moves the camera to fixed viewpoints and saves screenshots
 * to {@code screenshots/slashtracks-*.png}, then quits. Does nothing unless the property is set.
 */
public final class VisualTest {
    ...
    private static void build() {
        ...
        cmd(20, "execute positioned 7 -60 10 run slashtracks testtrack staircase 3",
                "execute positioned 7 -60 40 run slashtracks testtrack staircase 3 smooth",
                "execute positioned 7 -60 80 run slashtracks testtrack arc 12 smooth",
                "execute positioned 67 -60 40 run slashtracks testtrack loop 8 smooth",
        ...
        cmd(10, "give @s slashtracks:track_smoother", "item replace entity @s weapon.mainhand with slashtracks:track_smoother");
    ...
    private static void shot(int delay, String name) {
        step(delay, () -> {
            Minecraft mc = Minecraft.getInstance();
            Screenshot.grab(mc.gameDirectory, "slashtracks-" + name + ".png", mc.getMainRenderTarget(),
                    msg -> SlashTracks.LOG.info("[visualtest] {}", msg.getString()));
        });
    }
