...
/**
 * {@code config/slashtracks.properties}. Server keys apply on the server; client keys on the client.
...
public final class Config {
    ...
    static void load() {
        Path file = Platform.get().configDir().resolve("slashtracks.properties");
        Properties p = new Properties();
        if (Files.exists(file)) {
            try (Reader r = Files.newBufferedReader(file)) {
                p.load(r);
            } catch (IOException e) {
                SlashTracks.LOG.warn("Could not read {}, using defaults", file, e);
            }
        }
        ...
        try {
            ...
            try (Writer w = Files.newBufferedWriter(file)) {
                p.store(w, "SlashTracks. maxRunLength/opOnlyTool: server. debugOverlay/toolPreview: client.");
            }
        } catch (IOException e) {
            SlashTracks.LOG.warn("Could not write {}", file, e);
        }
    }
