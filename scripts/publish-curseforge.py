#!/usr/bin/env python3
"""
Publish SlashRails per-band JARs to CurseForge.

SlashRails ships one JAR per (MC version band, loader) — no platform variants.
...
# rejected with errorCode 1021. SlashRails is required on both sides (it draws the curves and
# moves carts on the client too), so every file is tagged Client and Server.
...
def discover_jars(release_dir: Path, mod_version: str) -> dict[tuple[str, str], Path]:
    ...
    for jar in sorted(release_dir.glob(f"slashrails-{mod_version}+mc*.jar")):
...
def upload_band(
    ...
    metadata = {
        ...
        "displayName": f"slashrails-{mod_version}+mc{band}-{loader}.jar",
